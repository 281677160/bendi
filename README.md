...
答：OP_DIY文件跟openwrt文件是同一个目录里面的，用WinSCP文件管理工具或者其他相同功能工具连接ubuntu，然后根据提示进入OP_DIY/xxx_source源码文件夹里面设置好里面的自定义文件跟配置文件，以后每次编译都读取里面的文件内容为主的，包括配置文件，如果你编译的时候有更改配置的话，编译完成后记得根据提示去更新配置文件。<br />
#
